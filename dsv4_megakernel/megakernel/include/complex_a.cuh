// ============================================================
//  complex_a.cuh — PURE OPERATOR (no main / no benchmark)
...
//
//  [TC/CC DUAL-PATH] Optional HC post+comb tail (fp32): one extra CUDA-core warp
//  (warp 4) computes hc_fused_kernel_tc's POST gate + Sinkhorn COMB concurrently
//  with the GEMM warps, hidden under this op's weight-read window -- the
//  mqa_logits_fp4 / complex_b pattern (no shared barriers: the GEMM path's
//  block-wide syncs are a role-scoped NamedBarrier over its 128 threads).
//  post/comb are consumed only by hc_post AFTER the attention block, so they are
//  OFF this op's critical path. The tail takes the ALREADY-REDUCED + rms-folded
//  mix [m,24] (the hc epilogue exports it -- one tiny fp32 store from its smem),
//  so its global traffic is ~80B in / 80B out per position: pure CUDA-core
//  compute (shuffles + exp), zero DRAM contention with the GEMM's TMA stream.
//  Disabled (tail warp exits) when HcTailArgs.mix == nullptr.
//
...
// ------------------------------------------------------------
// [TC/CC DUAL-PATH] HC post + Sinkhorn-comb tail (fp32), folded from
// hc_fused_kernel_tc.cu::hc_reduce_and_fuse_kernel (post/comb branches only;
// pre + collapse STAY in the hc epilogue -- they are on ITS critical path).
// The tail consumes the ALREADY-REDUCED + rms-FOLDED mix [m,24] (split-K reduce
// + Σx² rms are done upstream by the hc epilogue, which has mix in smem anyway
// and exports it with one tiny store) -- so the tail is pure CUDA-core compute:
// activation + 20-iter Sinkhorn, ~80B read + 80B written per position.
// Discipline (mqa_logits_fp4 edit #5 / complex_b warps 8-15): the tail warp
//   * never shares a barrier with the GEMM warps (which sync via
//     fusenorm_gemm_sync() = NamedBarrier(128, id 1); __syncthreads is banned),
//   * touches no SMEM / TMEM / mbarrier state -> starts at CTA entry,
//   * idles (returns) when hc.mix == nullptr.
// Everything here is fp32 in AND out (mix, base/scale, post/comb).
// ------------------------------------------------------------
namespace hc_tail {
...
// Thread roles: 4 GEMM warps (TMA producer / UMMA consumer / epilogue) + 1 tail warp.
constexpr int FUSENORM_GEMM_THREADS = 128;
constexpr int FUSENORM_TAIL_THREADS = 32;

struct HcTailArgs {
  const float* mix      = nullptr;   // [m, 24] REDUCED + rms-folded mix (fp32, upstream-produced)
...
// Role-scoped block sync for the 128 GEMM threads ONLY (hardware barrier 1; the
// tail warp never arrives here -- __syncthreads would hang/UB with it running).
__device__ __forceinline__ void fusenorm_gemm_sync() {
  cutlass::arch::NamedBarrier(FUSENORM_GEMM_THREADS, 1).sync();
}
...
// One warp handles one HC position (grid-strided): ONE coalesced 80B load of the
// already-reduced mix cols [4,24) -> post gate (lanes 0..3) -> warp-level Sinkhorn
// on the 4x4 comb (lanes 0..15, verbatim math from hc_reduce_and_fuse_kernel
// warp 0 -- all xor offsets < 16 keep lanes 0..15 closed; upper lanes carry
// masked garbage). No split-K reduce / Σx² here: done upstream.
__device__ static void hc_tail_run(HcTailArgs const& a) {
  ...
  for (int pos = blockIdx.x; pos < a.m; pos += gridDim.x) {
    ...
    // ---- comb: shift cols 8..23 (held by lanes 4..19) down to lanes 0..15, activate ----
    float v = __shfl_sync(0xffffffffu, mix, (lane + HC) & 31);
    v = (lane < HC * HC) ? v * s_comb + a.base[2 * HC + lane] : 0.f;

    // ---- Sinkhorn (softmax row-norm, then SINKHORN_ITERS-1 more row/col rounds) ----
    float max_v = v;
    #pragma unroll
...
gemm_device(ATensor mA, BTensor mB, DTensor mD,
            ...
            HcTailArgs hc_args)
{
  // [TC/CC] warp 4 = CUDA-core tail (HC post + Sinkhorn comb, fp32), hoisted ABOVE
  // the whole prologue: it touches no SMEM/TMEM/mbarrier state and never meets a
  // GEMM-path barrier (those are fusenorm_gemm_sync over 128 threads), so it starts
  // the instant the CTA does and exits independently. hc_args.mix==nullptr -> idle.
  if ((threadIdx.x >> 5) >= FUSENORM_GEMM_THREADS / 32) {
    if (hc_args.mix != nullptr)
      hc_tail_run(hc_args);
    return;
  }
  ...
  fusenorm_gemm_sync();   // GEMM warps only (tail exited above; __syncthreads banned)
  ...
  // no conflict). fusenorm_gemm_sync below is the correctness barrier (every GEMM
  // warp must observe barrier init + TMEM alloc before the producer issues TMA /
  // the consumer waits); the HC tail warp exited above and never arrives here.
  if (elect_one_warp) {
  ...
  fusenorm_gemm_sync();  // observe barrier init + TMEM alloc within this CTA (GEMM warps)
  ...
  for (int work = blockIdx.x; work < total_tiles; work += gridDim.x) {
    ...
    // ---- Rendezvous: MMA accumulator complete -> all GEMM warps enter the epilogue ----
    fusenorm_gemm_sync();
    ...
    fusenorm_gemm_sync();   // tile-end: all GEMM warps finished the epilogue before
                            // the next persistent tile re-inits/overwrites TMEM/SMEM.
...
using FuseNormLaunchFn =
    std::function<cutlass::Status(ElementC* dD, const float* rms_w, float eps,
                                  ...
                                  HcTailArgs const& hc, cudaStream_t stream)>;
                                        ...
                                        int M, int N, int K, int ks)
{
  ...
  dim3 dimBlock(FUSENORM_GEMM_THREADS + FUSENORM_TAIL_THREADS);   // 128 GEMM + 32 HC-tail
  ...
  return [=](ElementC* dD, const float* rms_w, float eps, SplitKPlan const& plan,
             float alpha, HcTailArgs const& hc, cudaStream_t stream) -> cutlass::Status {
...
// hc: OPTIONAL fp32 HC post+comb tail bundle (default = disabled). When hc.mix is
// set, the extra CUDA-core warp computes post[m,4] + Sinkhorn comb[m,4,4] from
// the ALREADY-REDUCED mix [m,24], hidden under this GEMM (TC/CC dual-path).
static void fusenorm_run(FuseNormCtx& ctx, bf16_t* out, const float* rms_w,
                         float eps = 1e-6f, cudaStream_t stream = 0,
                         HcTailArgs const& hc = HcTailArgs{}) {
  if (!ctx.valid || !ctx.launch_fn) return;
  ctx.launch_fn(reinterpret_cast<ElementC*>(out), rms_w, eps, ctx.plan, 1.0f, hc, stream);
}
