#pragma once
// ============================================================
// topk_v2.cuh — SELF-CONTAINED DeepSeek-V4 (DSA indexer) top-k device classes,
// migrated from sglang:
//   python/sglang/kernels/jit/include/sgl_kernel/deepseek_v4/topk_impl.cuh
// (which in turn credits vllm persistent_topk.cuh + flashinfer topk.cuh).
//
// Runs AFTER the score-attention kernel (mqa_logits_fp4 decode logits [B,T]
// fp32, stride % 4 == 0) as a SEPARATE launch — no fusion. The __global__
// kernels + host dispatcher + torch binding live in kernels/topk_v2.cu.
//
// Same self-contained discipline as mqa_logits_fp4.cuh: the tiny sgl_kernel
// helper closure (SGL_DEVICE / kWarpThreads / AlignedVector / warp::reduce_sum
// / PDL / div_ceil / enable_smem_spilling) is INLINED below, so this header
// depends only on the CUDA toolkit. The device::topk namespace and the
// algorithm body are kept verbatim where possible.
//
// [MEGAKERNEL EDIT]s vs upstream (all marked):
//   1. sgl_kernel includes -> inlined mini-closure (this header, top section).
//   2. C++20 -> C++17: [[likely]] dropped; fp16 casts use __half intrinsics
//      directly (sgl type.cuh's cast<> machinery not ported).
//   3. warp::reduce_sum: minimal template (u32 full-warp -> __reduce_add_sync,
//      group/other -> xor-shuffle tree) instead of sgl warp.cuh's ReductionOp.
// Everything else (histogram/threshold/collect/tie machinery, Register /
// Streaming / Cluster paths) is byte-identical in logic to upstream.
//
// Algorithm: fp16 coarse histogram -> threshold bin -> fp32-boundary collect ->
// exact radix tie-break. topk is a RUNTIME value <= kMaxTopK(2048); DSV4 decode
// uses index_topk = 512.
//
// NOTE __launch_bounds__(1024, 2) caps the kernel at 32 regs/thread; upstream
// leans on CUDA 13's `enable_smem_spilling` pragma (guarded no-op on 12.x —
// ptxas then spills to local, which upstream ships and tolerates).
// ============================================================
...
// ============================================================
// [MEGAKERNEL EDIT] inlined mini-closure from sgl_kernel/{utils,vec,warp}.cuh
// ============================================================
...
/// PDL (Programmatic Dependent Launch): wait for the primary kernel's trigger.
/// No-op when kUsePDL == false or pre-Hopper.
...
/// PDL: allow the dependent (secondary) kernel to launch.
template <bool kUsePDL>
...
/// (CUDA 13+ only; silently absent on 12.x — see the header note).
...
/// Aligned vector for wide (vectorized) global loads/stores. Trimmed from
/// sgl vec.cuh: only load / fill / operator[] are used by the topk paths.
...
#pragma unroll
  ...
  SGL_DEVICE T& operator[](std::size_t idx) { return data_[idx]; }
  SGL_DEVICE T operator[](std::size_t idx) const { return data_[idx]; }
};

namespace warp {

/// Warp sum reduction over groups of kNumThreads consecutive lanes (result
/// valid in every participating lane). Full-warp 32-bit integer sums use the
/// sm_80+ redux instruction; everything else takes the xor-shuffle tree.
...
SGL_DEVICE T reduce_sum(T value) {
  static_assert(kNumThreads >= 1 && kNumThreads <= kWarpThreads &&
                    (kNumThreads & (kNumThreads - 1)) == 0,
                "kNumThreads must be a power of two <= 32");
  if constexpr (kNumThreads == kWarpThreads && std::is_integral<T>::value && sizeof(T) <= 4) {
...
// ============================================================
// device::topk — the implementation classes (verbatim from sglang
// topk_impl.cuh modulo the marked C++17 / intrinsic edits)
// ============================================================
...
/// impl `Smem` types (`&smem` converts to the void* the forwards expect).
...
SGL_DEVICE uint32_t extract_coarse_bin(float x) {
  ...
  // [MEGAKERNEL EDIT] direct __half intrinsics (was sgl cast<fp16_t>)
...
SGL_DEVICE float coarse_bin_lower_bound(uint32_t bin) {
  ...
  const auto to_finite_val = [](uint32_t okey) -> float {
    ...
    // [MEGAKERNEL EDIT] direct __half2float (was sgl cast<float>(fp16_t))
    return __half2float(*reinterpret_cast<const __half*>(&hb));
  };
  // Fast path: `key` and `key - 1` both land in the finite band [0x0401, 0xFBFF].
  // fp16 rounds to nearest, so the fp32 boundary is the midpoint between the
  // fp16 values at `key` and `key - 1`. (Verified bit-exact upstream for every
  // bin of kBits 10 and 12.)
  ...
  // Ordered-key line: [0, 0x03FF) negative-NaN space, 0x03FF = -inf,
  // [0x0400, 0xFC00) finite, 0xFC00 = +inf, (0xFC00, 0xFFFF] positive-NaN
  // space. Treat +/-inf keys as +/-65536 and saturate NaN-space keys, keeping
  // boundaries finite-or-inf and monotone (rows containing >= topk inf scores
  // would otherwise come back short and illegal-address downstream).
...
struct TopKProblem {
  ...
  // by transform_output() in a separate, pipelined pass (keeps the per-element
  // page_table gather off the atomic-serialized scatter loop).
...
struct TopKConfig {
  ...
  // kMaxNumTie must be >= kMaxTopK: up to `topk` output slots may have to be
  // filled from the tie candidates (above_count can be 0, e.g. heavily tied or
  // all-inf scores); padded slots inside min(seq_len, topk) are dereferenced
  ...
  SGL_DEVICE static void handle_tie(  //
    ...
    if (num_ties <= topk) {
      ...
      // 128 x 128 topk implementation: each thread takes 4 elements
      const auto invalid = TieValue::invalid();
      const TieValue tie[] = {
...
#pragma unroll
      ...
      // Rare overflow case (kBlockSize < num_ties <= kMaxNumTie).
...
struct TopKRadixBase : TopKConfig {
  ...
  struct Smem {
    ...
    // threshold_bin; the tie machinery only comes alive after that. Overlaying
    // the two phases keeps the kMaxNumTie-candidate buffer from growing the
    // block's shared-memory footprint. tie.handle and tie.values are live
    // TOGETHER, so they sit side by side inside the overlay.
    union {
...
struct TopKRegister : TopKRadixBase<12> {
  ...
  SGL_DEVICE static void forward(const TopKProblem problem, void* _smem) {
    ...
    // vector-loaded. The <kVecSize tail is a scalar remainder on the LAST lanes.
...
#pragma unroll
    ...
    const auto collect = [&](float val, uint32_t idx) {
      if (val >= v_hi) {
        ...
        if (count_eq < kMaxNumTie) smem->tie.values[count_eq] = {val, idx};
      }
    };
#pragma unroll
    for (uint32_t i = 0; i < kLocalVecs; ++i) {
...
// ---------------------------------------------------------------------------
// Streaming path: seq_len > 16384 -- two vectorized passes over global memory
// ---------------------------------------------------------------------------
...
struct TopKStreaming : TopKRegister<2> {
 public:
  ...
  SGL_DEVICE static void forward(const TopKProblem problem, void* _smem) {
    ...
    // Phase 3: Collect candidates. Classify by two fp32 boundaries derived from
    // the threshold bin (drops the F2F + bit-twiddle from the second full pass).
    const auto threshold_bin = smem->threshold_bin;
    ...
    // Phase 4: Handle ties. Output layout driven by the *collect* counts so it
    // is self-consistent with the fp32 classification above.
    __syncthreads();
    const auto above_count = smem->count_gt;
    const auto equal_count = smem->count_eq;
    const auto remain_topk = above_count < topk ? topk - above_count : 0;
...
struct TopKCluster : TopKRadixBase<10> {
 public:
  ...
  SGL_DEVICE static void forward(TopKProblem problem, void* _smem) {
    ...
    // Phase 1.5: reduce the histogram across the cluster (1-shot all-reduce:
    // each rank owns kPartition consecutive bins; gather the kClusterSize peer
    // values via DSMEM, sum, scatter back).
    {
      cluster.sync();
    ...
    // back to global by adding chunk_start. threshold_bin is identical across
    // ranks, so v_hi/v_lo are too.
    const auto topk = problem.topk;
    const auto threshold_bin = smem->threshold_bin;
    const float v_hi = coarse_bin_lower_bound<kHistBits>(threshold_bin + 1);
    const float v_lo = coarse_bin_lower_bound<kHistBits>(threshold_bin);
