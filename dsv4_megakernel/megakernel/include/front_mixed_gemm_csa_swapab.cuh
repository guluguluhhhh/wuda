...
// FP8 and BF16 both use UMMA_M=128. UMMA_N is the batch tile.
// N-parallelism contributes 37 feature clusters per batch tile (16 fp8 N128
// + 21 bf16 N128, with 64 valid rows in the BF16 tail tile). The host uses one
// batch tile through M=48 and two tiles above it to occupy all 148 B300 SMs.
...
constexpr int FP8_CTA_N = FP8_UMMA_M / CLUSTER_SIZE;      // 64 features/CTA
constexpr int BF16_CTA_N = BF16_UMMA_M / CLUSTER_SIZE;    // 64 features/CTA
constexpr int FP8_FEATURE_TILES = N_FP8 / FP8_UMMA_M;     // 16 clusters
...
constexpr int SF_WEIGHT_BYTES = SF_GROUPS * SF_ROWS * sizeof(uint32_t);
// M128 cta_group::2 uses the 2x2 SFB allocation. Its two BatchN/2 quadrants
// require separate 512-byte source patterns for each four-scale K group.
constexpr int SF_ACT_BYTES = 2 * SF_WEIGHT_BYTES;
...
template <int BatchN>
struct Config {
  static_assert(BatchN == 16 || BatchN == 32 || BatchN == 48 ||
                BatchN == 64 || BatchN == 80 || BatchN == 96 ||
                BatchN == 128);
  ...
  // Keep the deepest pipeline that fits the 227.5KB shared-memory cap.
  static constexpr int FP8_STAGES = BatchN < 64 ? 5
      : BatchN < 128 ? 10 : 12;
  static constexpr int BF16_STAGES = BatchN <= 48 ? 5
      : BatchN == 64 ? 9
      : BatchN <= 96 ? 8 : 7;
  ...
  static constexpr int FP8_STORAGE_BYTES =
      FP8_PIPELINE_BYTES + SF_WEIGHT_BYTES + SF_ACT_BYTES;
  static constexpr int STORAGE_BYTES = FP8_STORAGE_BYTES > BF16_PIPELINE_BYTES
      ? FP8_STORAGE_BYTES : BF16_PIPELINE_BYTES;

  // Accumulators occupy BatchN columns; place scale factors after them.
  static constexpr int SF_WGT_COL = BatchN <= 64 ? TMEM_SF_WEIGHT : BatchN;
  static constexpr int SF_ACT_COL = BatchN <= 64 ? TMEM_SF_ACT : BatchN + 4;
  static constexpr int NUM_TMEM_COLS = BatchN <= 96 ? 128 : 256;
};
...
struct SharedStorage {
  ...
  // storage can overlap and the larger specialization sets the allocation.
...
// SFB copy for the M128 2x2 datapath. Unlike the 4x1 SFA copy, the two
// BatchN/2 scale patterns remain distinct in TMEM instead of being broadcast
// across all four 32-datapath subpartitions.
__device__ __forceinline__ void utccp_sfb_2x2(
    uint32_t tmem_col, uint64_t sf_desc) {
  asm volatile(
...
static __global__ void __launch_bounds__(TPB, 1) swapab_kernel(
  ...
  uint8_t* const sf_act = sf_weight + SF_WEIGHT_BYTES;
  ...
  // Resident FP8 scale factors. The cluster owns one 128-feature weight-scale
  // block; its two 64-feature CTAs therefore load the same weight scale. Both
  // CTAs also replicate the full UMMA_N activation scale tile. Rows outside
  // the logical batch are padded to 1.0.
  if (warp == 3 && is_fp8) {
    ...
    for (int slot = lane; slot < SF_ROWS; slot += 32) {
      #pragma unroll
      for (int g = 0; g < SF_GROUPS; ++g) {
        base::st_shared_u32(sf_weight + (g * SF_ROWS + slot) * 4,
                            packed_w[g]);
        base::st_shared_u32(
            sf_act + (g * 2 * SF_ROWS + slot) * 4, UE8M0_ONE4);
        base::st_shared_u32(
            sf_act + (g * 2 * SF_ROWS + SF_ROWS + slot) * 4,
            UE8M0_ONE4);
      }
    }
    #pragma unroll
    for (int l = lane; l < BatchN; l += 32) {
      const int row = batch_base + l;
      const int half = l / (BatchN / CLUSTER_SIZE);
      const int local_l = l % (BatchN / CLUSTER_SIZE);
      const int slot = (local_l % 32) * 4 + local_l / 32;
      #pragma unroll
      for (int g = 0; g < SF_GROUPS; ++g) {
        const uint32_t av = row < problem_m
            ? *reinterpret_cast<const uint32_t*>(
                  x_sf + static_cast<size_t>(row) * FP8_SCALE_TILES + g * 4)
            : UE8M0_ONE4;
        base::st_shared_u32(
            sf_act + (g * 2 * SF_ROWS + half * SF_ROWS + slot) * 4, av);
      }
    }
        ...
        for (int k_tile = 0; k_tile < C::FP8_K_TILES; ++k_tile) {
          ...
          for (int half = 0; half < C::FP8_BLOCK_K / FP8_TMA_K; ++half) {
            ...
            if ((scale_tile & 3) == 0) {
              ...
              fp8d::replace_sf_desc_addr(
                  sfd, sf_act + g * 2 * SF_ROWS * 4);
              utccp_sfb_2x2(C::SF_ACT_COL, fp8d::sf_desc_bits(sfd));
            }
  ...
  if (is_fp8) {
    ...
    if (warp >= 4) {
      ...
      // selects one 32-feature chunk, and the high bit selects BatchN/2 rows.
      const int epi_warp = warp - 4;
      const int out_col = feature_base + rank * FP8_CTA_N +
                          (epi_warp & 1) * STORE_N + lane;
      #pragma unroll
