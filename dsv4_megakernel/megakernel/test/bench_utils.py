"""
Vendored VERBATIM from DeepGEMM deep_gemm/testing/bench.py (bench_kineto +
stdout suppressors). Self-contained -- no deep_gemm install needed.

Shared timing utility for ALL operator tests in this repo:
    from bench_utils import bench_kineto
    t = bench_kineto(lambda: my_op(args), 'my_kernel_name_substr',
                     suppress_kineto_output=True)          # seconds

Semantics: 8GB memset before EVERY call (cold L2 + GPU 'chill time' so each
instance starts at boost clocks), kineto kernel device-time only, first
profiler cycle discarded as warmup, MEAN over instances. Pass a tuple of
names when one call launches several kernels (e.g. cuBLASLt split-K
('nvjet', 'reduce')); a missing name yields 0.
"""
import os
import sys
import torch
